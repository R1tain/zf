...
void handle_tcp_connection(const Session *session, int client_fd, FILE *log_fp) {
    ...
    while (1) {
        ...
        if (nfds == 0) { // Timeout
            log_message(log_fp, session->id, "INFO", "Connection on fd %d idle timeout.", client_fd);
            break;
        }
...
void *check_connection_thread(void *arg) {
    ...
    while (!g_quit_flag) {
        ...
        if (sock < 0) {
            ...
            if (fail_count > 0) {
                 log_message(f->log_fp, f->session.id, "INFO", "Health check successful, connection to remote host restored.");
            }
            fail_count = 0;
            close(sock);
        }
...
void start_forwarding(PortForwarder *f) {
    ...
    log_message(f->log_fp, f->session.id, "INFO", "Main listener started on PID %d. Waiting for connections...", getpid());
    while (!g_quit_flag) {
        int client_fd = accept(listen_fd, NULL, NULL);
        if (client_fd < 0) {
            if (errno == EINTR && g_quit_flag) break;
            if (errno == EINTR) continue;
        ...
        if (pid == 0) { // In Child Process
            // Child should not handle parent's signals
            struct sigaction sa_child = {0};
            sa_child.sa_handler = signal_handler_child;
            sigaction(SIGTERM, &sa_child, NULL);
            sigaction(SIGINT, &sa_child, NULL);
            sigaction(SIGCHLD, &sa_child, NULL); // Child doesn't manage its own children
    ...
    log_message(f->log_fp, f->session.id, "INFO", "Shutdown signal received. Closing listener.");
    if (f->check_thread) {
    ...
    close(listen_fd);
    if (f->control_fd >= 0) close(f->control_fd);
    kill(0, SIGTERM); // Kill all processes in the process group
}

void list_sessions() {
    DIR *dir = opendir(SESSION_DIR);
    if (!dir) {
        fprintf(stderr, "Cannot open session directory %s: %s\n", SESSION_DIR, strerror(errno));
        return;
    }
...
void handle_control_requests(PortForwarder *f) {
    ...
    if (n > 0) {
        ...
        if (strcmp(buffer, "kill") == 0) {
            log_message(f->log_fp, f->session.id, "INFO", "Received kill command via control socket.");
            kill(f->session.pid, SIGTERM);
        }
    }
    close(client_fd);
...
int main(int argc, char *argv[]) {
    ...
    if (getenv("ZF_DAEMON") == NULL) {
        ...
        umask(0);
        if (setsid() < 0) { exit(1); } // setsid can fail
