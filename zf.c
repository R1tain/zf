...
DynamicBuffer *create_buffer(void) {
...
void cleanup(PortForwarder *f) {
    if (f->running) {
        ...
        if (f->stats != MAP_FAILED) {
            munmap(f->stats, sizeof(Stats));
            f->stats = MAP_FAILED;
        }
    }
}
...
void delete_session(PortForwarder *f) {
    ...
    if (unlink(session_path) < 0 && errno != ENOENT) {
        log_message(f, "删除会话文件 %s 失败: %s", session_path, strerror(errno));
    } else {
        log_message(f, "已删除会话文件: %s", session_path);
    }
    if (unlink(sock_path) < 0 && errno != ENOENT) {
        log_message(f, "删除套接字文件 %s 失败: %s", sock_path, strerror(errno));
    } else {
        log_message(f, "已删除套接字文件: %s", sock_path);
    }
}
...
int init_session(PortForwarder *f) {
    ...
    f->check_thread = 0;
    f->quality_thread = 0;
    f->stats = mmap(NULL, sizeof(Stats), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (f->stats == MAP_FAILED) {
        log_message(f, "创建共享内存失败: %s", strerror(errno));
        return -1;
    }
